"""
Чтобы исключение дошло до нас, задачу нужно вызывать в выражении await. Для демонстрации предположим, что вместо игнорирования задач
...
tasks_list = []                                                         # вот как раз из за списка мы не перехватим исключение!
async def listen_for_connection(server_socket: socket,
    ...
    while True:
        ...
        tasks_list.append(asyncio.create_task(echo(connection, loop)))                 # После получения запроса на подключение создаем задачу echo, ожидающую данные от клиента и сохраняем в список
